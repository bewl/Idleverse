...
- The tutorial window is now draggable from the full header region, with only a small centered grip indicator instead of a labeled drag control, while interactive header controls such as `Skip Tour` remain clickable and do not trigger drag capture.
