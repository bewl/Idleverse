...
```
    ...
    `SystemPanel` now splits into a presentation shell plus a dedicated `SystemSceneCanvas` renderer. The panel still owns browsing state, selected-body state, anomaly mode, docking/POS actions, mining-assignment detail, and the derivation of local convoy contacts, while the new canvas owns constrained orbit camera controls, hit-testing, and scene rendering for bodies, fleets, and local structures. Arrival presentation is now simulation-backed: fleets and hauling wings record `recentTransitArrival` snapshots in save-backed state, and the canvas only consumes that state instead of inventing arrival previews on its own. The renderer now fills the scene shell by explicit parent-bounds positioning instead of relying on flex sizing from the outer panel, which keeps the compact System view from collapsing to a partial-height canvas. This keeps the operational logic in the panel while isolating the camera-heavy rendering path into its own file.

    The responsive shell for `SystemPanel` now follows the same mobile-first rule as `StarMapPanel`: desktop keeps the fixed inspector rail, while compact or coarse-pointer layouts keep the scene primary and move inspector content into a slide-up sheet. Hover-only scene intel is treated as desktop behavior; compact layouts rely on tap selection and pinned fleet/body state instead. The compact scene overlay itself now collapses its top intel pills into a reduced top-right stacked column so canvas telemetry stays readable without covering the scene on phone widths. Touch input now mirrors the Galaxy Map interaction model inside `SystemSceneCanvas`: one-finger drag orbits the scene, two-finger gestures handle pan plus pinch zoom, and tap selection replaces cursor hover. Visual rendering in the System scene is now more camera-sensitive as well: planets, belts, the central star, local fleet markers, convoy markers, and structure markers scale with projected depth, while hit targets keep their previous readability floor so touch usability does not collapse when the camera zooms out. The `Anomalies` tab stays in the same panel slice, but its scanner controls and action buttons reflow into a narrow-safe single-column command surface rather than trying to preserve desktop row density.
...
The tutorial uses the same split intentionally. Canonical tutorial progress lives in save-backed `GameState.tutorial` so onboarding is scoped to each save, while the active overlay visibility is stored in `uiStore.tutorialOverlayOpen` because the shell is still a presentation concern. The onboarding shell itself is now behind a temporary feature flag in `src/game/progression/tutorialSequence.ts`: while that flag is off, new or restarted saves initialize straight into a skipped tutorial state and `GameLayout` suppresses the resume control instead of presenting a dead onboarding affordance. When the flag is re-enabled, the same shell still drives cross-panel Fleet, Star Map, System, and Mining steps by reusing `panelStates` and `focusTarget` rather than introducing a separate tutorial-only router.
